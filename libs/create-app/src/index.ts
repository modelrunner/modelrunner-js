...
const repoUrl =
  "https://github.com/modelrunner/modelrunner-nextjs-template.git";
...
async function main() {
  ...
  try {
    ...
    program
      .name("The modelrunner.ai App Generator")
      .description(
        "Generate full stack AI apps integrated with modelrunner.ai",
      );
    ...
    const hasModelrunnerEnv = await select({
      message: "Do you have a modelrunner.ai API key?",
      choices: [
        {
          name: "Yes",
          value: true,
    ...
    if (!hasModelrunnerEnv) {
      await open("https://www.modelrunner.ai/dashboard");
    }

    const modelrunner_api_key = await input({
      message: "modelrunner.ai API Key",
    });
    ...
    # MODELRUNNER AI API Key
    MODELRUNNER_KEY="${modelrunner_api_key}"
    `;
