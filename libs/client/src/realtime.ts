...
export interface RealtimeClient {
   ...
   * WebSockets to connect to modelrunner function endpoints that support WSS.
   *
...
function buildRealtimeUrl(
  ...
  return `wss://modelrunner.run/${appId}${normalizedPath}?${queryParams.toString()}`;
}
...
function isSuccessfulResult(data: any): boolean {
  return (
    data.status !== "error" &&
    data.type !== "x-modelrunner-message" &&
    !isModelrunnerErrorResult(data)
  );
}

type ModelrunnerErrorResult = {
  type: "x-modelrunner-error";
  error: string;
  reason: string;
};

function isModelrunnerErrorResult(data: any): data is ModelrunnerErrorResult {
  return data.type === "x-modelrunner-error";
}
...
function handleRealtimeMessage({
  ...
  const handleDecoded = (decoded: any) => {
    ...
    // TODO: specify the modelrunner ws protocol format
    ...
    if (isModelrunnerErrorResult(decoded)) {
